...
for i in range(16):
    ...
    xorResult=xor.operatingExclusive(extendPlain,keyPressPermutation2) #평문48bit xor 키
    sBoxResult=sBox.sBoxRepalce(sBox,xorResult) #s-box결과
    pBoxResult=pBox.pBoxReplace(sBoxResult) #p-box결과
    roundR32Bit=xor.operatingExclusive(fristPermutationResult,pBoxResult) #pbox결과 xor평문
    ...
    saveResult.append(fristPermutationResult)
else:
    saveResult[-1][0],saveResult[-1][1]=saveResult[-1][1],saveResult[-1][0] #마지막 라운드 서로 바꿈

for i in saveResult: # 1~16라운드 결과
    print("[{0} 라운드 결과]".format(count+1))
    xor.displayBit(i)  
    count+=1
#최종 순열 결과
print("[64bit 블록 암호화 결과]")
xor.displayBit(xor.FinalPermutation(saveResult)) #최종 순열 거친 뒤 결과 출력 
