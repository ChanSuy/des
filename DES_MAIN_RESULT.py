...
fristPermutationResult = fristPermutation.fristPermutationMapping(
    fristPermutation, binPlain
)  # 초기순열결과 저장
...
for i in range(16):
    ...
    extendPlain = fristPermutation.extendPermutation(
        fristPermutationResult
    )  # 32bit 평문 확장
    xorResult = calculation.operatingExclusive(
        extendPlain, keyPressPermutation2
    )  # 평문48bit calculation 키
    sBoxResult = sBox.sBoxRepalce(sBox, xorResult)  # s-box결과
    pBoxResult = pBox.pBoxReplace(sBoxResult)  # p-box결과
    roundR32Bit = calculation.operatingExclusive(
        fristPermutationResult, pBoxResult
    )  # pbox결과 xor평문
    keyPressPermutation1 = keyShift  # 시프트 결과를 저장하여 다음 시프트 함수에 넣기위한 변수
    fristPermutationResult = [
        fristPermutationResult[1],
        roundR32Bit[0],
    ]  # 평문 오른쪽 32bit를 왼쪽 평문,마지막 xor결과를 오른쪽 평문으로
    saveResult.append(fristPermutationResult)  # 결과를 하나의 리스트에 저장
else:
    saveResult[-1][0], saveResult[-1][1] = (
        saveResult[-1][1],
        saveResult[-1][0],
    )  # 마지막 라운드 서로 바꿈
a, b = b, a
...
print("[암호문 문자열 결과]")
print(calculation.changeChr(finalRoundResult))
